...
export function LogoRow({ label, logos, invert = false }: Props) {
  return (
    <section
      ...
      aria-label={label}
    >
      <div className="section-shell py-8 md:py-11">
        <p
          className={`mb-6 text-center text-[11px] font-semibold uppercase tracking-section ${
            invert ? "text-white/50" : "text-forest"
          }`}
        >
          {label}
        </p>
        <ul className="grid grid-cols-2 items-center justify-items-center gap-x-6 gap-y-5 md:grid-cols-4 md:gap-x-10 md:gap-y-7">
          {logos.map((logo) => (
            <li
              key={logo.src}
